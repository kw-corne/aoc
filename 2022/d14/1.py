...
def drop_sand(drop_point: Point, rocks: Rocks, bounds: Bounds) -> bool:
    if not is_point_in_bounds(drop_point, bounds):
        return False
    ...
    if rocks[(x - 1, y + 1)] == ".":
        return drop_sand((x - 1, y + 1), rocks, bounds)
    elif rocks[(x + 1, y + 1)] == ".":
        return drop_sand((x + 1, y + 1), rocks, bounds)
    else:
        rocks[(x, y)] = "o"
...
if __name__ == "__main__":
    ...
    i = 0
    while True:
        if drop_sand(sand_drop_point, rocks, bounds):
            i += 1
        else:
            print(i)
            exit()
